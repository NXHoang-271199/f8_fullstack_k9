...
recognition.onresult = function(event) {
  ...
  setTimeout(() => {
    ...
    } else if (transcript.includes('google maps') || transcript.includes('bản đồ')) {
      window.open('https://www.google.com/maps', '_blank');
    } else if (transcript.includes('chỉ đường') || transcript.includes('tới') || transcript.includes('đường tới')) {
      const destination = transcript.split('tới')[1] || transcript.split('đường tới')[1];
      window.open(`https://www.google.com/maps/search/${destination.trim()}`, '_blank');
    } else if (transcript.includes('bài hát')) {
      const songName = transcript.split('bài hát')[1].trim();
      window.open(`https://mp3.zing.vn/tim-kiem/bai-hat.html?q=${songName}`, '_blank');
    } else if (transcript.includes('mở bài hát') || transcript.includes('nghe bài hát')) {
      const songName = transcript.split('nghe bài hát')[1].trim() || transcript.split('mở bài hát')[1].trim();
      window.open(`https://mp3.zing.vn/tim-kiem/bai-hat.html?q=${songName}`, '_blank');
    } else if (transcript.includes('video')) {
      const videoName = transcript.split('video')[1].trim();
      window.open(`https://www.youtube.com/results?search_query=${videoName}`, '_blank');
    } else {
...
//     actionEl.textContent = 'Lỗi nhận diện giọng nói. Vui lòng thử lại.';
// };
